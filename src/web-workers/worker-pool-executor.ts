...
class WorkerPool {
  ...
  constructor(numWorkers: number) {
    ...
    for (let i = 0; i < numWorkers; ++i) {
      const worker = new Worker('/workerBridge.js'); // <<< Worker / Worker
      workers.push(worker);
  ...
  public createContext<C>(
    ...
    return {
      ...
      value,
      transferList: transferList as any
    };
  }
...
class WorkerExecuteExecution<I, O, C> {
   ...
   */
  private readonly transferList?: Array<Transferable>;
  ...
  start() {
    ...
    const combinedTransferList: Array<Transferable> = this.transferList
    ...
    return ExecutorPromise.forExecutor<unknown, O>(
      ({
        ...
        execPort.addEventListener('messageerror', e => {
          rejectAll(e);
          execPort.close();
        });

        // Register handler for regular messages
        execPort.addEventListener('message', (event: MessageEvent) => {
...
class WorkerMapExecution<I, O, C> {
  ...
  private __nextWorkerSession(
    ...
    session.execPort.addEventListener('message', (event: MessageEvent) => {
      const message = event.data as CommandResult | CommandError;
      if (this.isAborted) {
        return;
      }
...
export default class WorkerPoolExecutor {
  ...
  public execute<I, O>(
    ...
    data: I,
    transferList: Array<Transferable> = undefined
  ) {
